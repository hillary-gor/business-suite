/**
 * The CA that signs hosted Supabase Postgres and pooler certificates.
 *
 * This is not a workaround and it is not a local trust exception. The
 * endpoints present a chain rooted at "Supabase Root 2021 CA", which is a
 * private root. Node's Mozilla bundle does not contain it, so
 * `rejectUnauthorized: true` with no extra CA fails with
 * `SELF_SIGNED_CERT_IN_CHAIN` — the root is self-signed, by design.
 *
 * The PEMs here match `certs/supabase-prod-ca-2021.crt` (intermediate, then
 * root). They are inlined so a serverless deploy does not have to find a
 * file on disk. Keep the two copies in step; the unit test asserts that.
 */
export const SUPABASE_PROD_CA_2021 = `-----BEGIN CERTIFICATE-----

VCgJ
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;
